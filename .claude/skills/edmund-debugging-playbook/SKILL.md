...
| Right-click on a toolbar button shows "Customize Toolbar…" | Expected since 2026-09-23: toolbar buttons carry no right-click menus (the `DocumentWindow.sendEvent` intercept was removed). A button that needs a menu gets a visible `NSMenuToolbarItem` pull-down | ARCHITECTURE §8 |
