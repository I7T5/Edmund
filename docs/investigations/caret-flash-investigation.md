...
| `burst ms,interval,dir` | background thread, `CGWindowListCreateImage` of **our window id only** — nothing else on screen is included — every ~13–35 ms, files named `NNNN-<uptime ms>.png`, timestamp taken *after* the capture returns. |
