...
| Read mode / Export | `Export/` — `HTMLRenderer` (MarkupVisitor → HTML; callout/checkbox icons are inline Lucide SVGs from `LucideIcons`), `HTMLTheme` (EditorTheme → CSS; the font cascade becomes one `@font-face { src: local("Family"); unicode-range: … }` block per configured script, with the cascade families leading `--body-font`), `DocumentHTML` (assembly + asset inlining: math + local images → data URIs), `ReadModeWebView`, `MarkdownPrinter` (PDF/Print). `Document.refreshReadView()` keeps an open Read view in sync with edits and theme changes. |
...
- **Six panes**, built by `SettingsWindowController.addPane`: General,
  Appearance, Fonts, Edit, Syntax, Advanced. Keys are namespaced to match
  ...
  `settings.syntax.*`. Fonts owns the per-script cascade
  (`EditorFontCascade` = `[script: family]` dict on `EditorTheme`;
  unset scripts keep the system fallback).
