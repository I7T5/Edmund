...
before they read it, not after. Use these lines verbatim — the header, then a
divider that separates it from the review:

```markdown
🤖 Generated by Claude Code & Reviewed by the dev 💁‍♀️

---
```

This header goes on review comments only, never on PR descriptions.

Then the review body, in the register of `edmund-docs-and-writing` §3 "PR
descriptions and review comments": the contributor knows it is
AI-generated, so no thanks, praise, softeners or sign-off. Verdict first,
then items grouped by disposition (merge / change / drop), each as problem →
evidence (`file:line`) → required fix.
