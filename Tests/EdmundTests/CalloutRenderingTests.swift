...
struct CalloutRenderingTests {
    ...
    @Test("Custom title is hidden, drawn in the header image")
    @MainActor func customTitleInImage() {
        let editor = makeEditor()
        ...
        // The title is baked into the header image, so its source is hidden.
    ...
    @Test("A long custom title wraps (a taller header image)")
    @MainActor func longCustomTitleWraps() {
        ...
        let styledShort = editor.styleBlock("> [!question] Short\n> body")
        let styledLong = editor.styleBlock("> [!question] \(longTitle)\n> body")
        func headerImageHeight(_ s: NSAttributedString) -> CGFloat {
            (s.attribute(.fragmentOverlay, at: 2, effectiveRange: nil) as? FragmentOverlay)?
                .bounds.height ?? 0
        }
        // The long title wraps inside the image → a taller header image.
        #expect(headerImageHeight(styledLong) > headerImageHeight(styledShort) + 1,
                "long title should make a taller (wrapped) header image")
    }
