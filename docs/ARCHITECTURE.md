...
| Status bar | `edmd/Views/StatusBarView.swift`, prefs in `Model/StatusBarPrefs.swift` (app-wide; `save` posts `didChangeNotification` so every window's bar follows). View ▸ Show/Hide Status Bar + Auto-Hide Status Bar mirror the toolbar pair; the bar's context menu picks fields only |
